...
import { PRICING_FEATURES, type FeatureStatus } from "@/lib/pricingFeatures";
      ...
      <div className="max-w-6xl mx-auto px-4 py-8 sm:py-12">
        ...
        {success && (
          <div className="mb-8 bg-green-500/10 border border-green-500/30 rounded-xl p-4 text-center">
            ...
            <div className="text-green-300/80 text-sm">Your account has been upgraded. Your new limits should be available shortly.</div>
          </div>
        )}
        {canceled && (
        ...
        <div className="text-center mb-10">
          ...
          <p className="text-muted text-base sm:text-lg max-w-2xl mx-auto">
            MonteCarloo is free forever. Upgrade for higher limits today, and see clearly which advanced features are live, in beta, or coming soon.
          </p>
        </div>
        ...
        <div className="grid grid-cols-1 md:grid-cols-3 gap-6 mb-12">
          ...
          <div className="bg-card border-2 border-accent rounded-2xl p-6 flex flex-col relative">
            ...
            <ul className="space-y-2.5 mb-6 flex-1">
              ...
              <Feature text="Multi-timeframe analysis" status="beta" />
              <Feature text="Advanced event templates" status="coming_soon" />
              <Feature text="Priority simulation queue" status="coming_soon" />
            </ul>
            {userTier === "pro" ? (
              <button
                onClick={handleManage}
              ...
              >
                ...
                ) : userTier === "premium" ? "Switch to Pro" : "Upgrade to Pro"}
              </button>
            )}
          </div>
          ...
          <div className="bg-card border border-border rounded-2xl p-6 flex flex-col">
            ...
            <ul className="space-y-2.5 mb-6 flex-1">
              ...
              <Feature text="API access" highlight status="coming_soon" />
              <Feature text="Priority support" highlight status="coming_soon" />
              <Feature text="Custom event templates" status="coming_soon" />
              <Feature text="White-label exports" status="coming_soon" />
              <Feature text="Early access to features" status="beta" />
            </ul>
            {userTier === "premium" ? (
              <button
                onClick={handleManage}
          ...
          <div className="flex flex-col sm:flex-row items-start sm:items-center justify-between gap-4">
            <div>
              ...
              <p className="text-sm text-muted max-w-lg">
                Enterprise is being shaped for teams that need programmatic simulation, higher limits, and support workflows.
                Advanced items below are planned unless explicitly marked live elsewhere.
              </p>
              <div className="flex flex-wrap gap-3 mt-3 text-xs text-muted">
                <span className="flex items-center gap-1"><span className="text-yellow-400">◌</span> REST API — coming soon</span>
                <span className="flex items-center gap-1"><span className="text-yellow-400">◌</span> Bulk sim — coming soon</span>
                <span className="flex items-center gap-1"><span className="text-yellow-400">◌</span> Custom events — coming soon</span>
                <span className="flex items-center gap-1"><span className="text-yellow-400">◌</span> Webhook delivery — coming soon</span>
                <span className="flex items-center gap-1"><span className="text-yellow-400">◌</span> White-label — coming soon</span>
                <span className="flex items-center gap-1"><span className="text-yellow-400">◌</span> SLA & support — coming soon</span>
              </div>
            </div>
            <a
            ...
            <table className="w-full">
              <thead>
                <tr className="border-b border-border">
                  ...
                  <th className="text-center text-sm text-accent font-medium px-4 py-3">Status</th>
                </tr>
              </thead>
              <tbody>
                {PRICING_FEATURES.map((f, i) => (
                  <tr key={i} className={`border-b border-border/50 ${f.highlight ? "bg-accent/5" : ""}`}>
                    ...
                    <td className="text-center text-sm px-4 py-2.5">
        ...
        <div className="mt-12 max-w-3xl mx-auto">
          ...
          <div className="space-y-4">
            ...
            <FAQ
              ...
              a="Pro unlocks unlimited events per scenario and unlimited Pine Script overlays today. Some advanced workflow features shown on this page may still be marked beta or coming soon."
            />
            <FAQ
              q="How do you label beta vs coming soon?"
              a="Live means available now. Beta means implemented but still evolving. Coming soon means planned or partially built, but not something we want you relying on in production yet."
            />
            <FAQ
              q="Can I cancel anytime?"
              a="Yes, cancel anytime from the billing portal. You'll keep your current paid access until the end of your billing period."
            />
            <FAQ
...
function Feature({ text, highlight = false, status = "live" }: { text: string; highlight?: boolean; status?: FeatureStatus }) {
  const iconColor = status === "coming_soon" ? "text-yellow-400" : status === "beta" ? "text-sky-400" : (highlight ? "text-accent" : "text-green-400");
  return (
    <li className="flex items-start gap-2 text-sm">
      <svg className={`w-4 h-4 mt-0.5 flex-shrink-0 ${iconColor}`} fill="none" viewBox="0 0 24 24" stroke="currentColor">
        <path strokeLinecap="round" strokeLinejoin="round" strokeWidth={2.5} d="M5 13l4 4L19 7" />
      </svg>
      <span className={highlight ? "text-white font-medium" : "text-muted"}>{text}</span>
      {status !== "live" && <StatusBadge status={status} compact />}
    </li>
  );
}

function StatusBadge({ status, compact = false }: { status: FeatureStatus; compact?: boolean }) {
  const styles = {
  ...
  const label = status === "coming_soon" ? "Coming soon" : status === "beta" ? "Beta" : "Live";
  return (
    <span className={`${compact ? "ml-2" : ""} inline-flex items-center px-2 py-0.5 rounded-full border text-[11px] font-medium ${styles[status]}`}>
      {label}
    </span>
  );
}
