...
${drawLines("p95_ts", "p95_px", "color.new(proj_color, 85)", 1, "line.style_dotted")}`;
  ...
  return `//@version=5
// ═══════════════════════════════════════════════════════════════
// MonteCarloo Event Simulation — ${ticker}
// Generated by MonteCarloo.io — Live event-driven stock simulation
// ═══════════════════════════════════════════════════════════════
//
...
//
// 🔗 Create your own: https://montecarloo.io/sim/${ticker}
//
indicator("MonteCarloo: ${ticker} Event Simulation", overlay=true, max_bars_back=500, max_lines_count=500)
...
${bandCode}
    ...
    table.cell(info, 0, 0, "MonteCarloo", text_color=proj_color, text_size=size.small)
    ...
    table.cell(info, 1, 5, "montecarloo.io", text_color=color.gray, text_size=size.tiny)
`;
}
...
export default function SimChart({
  ...
  useEffect(() => {
    ...
    if (pineOverlay && pineOverlay.plots.length > 0) {
      ...
      const isOverlay = pineOverlay.meta.overlay;

      for (const plotLine of pineOverlay.plots) {
        const seriesOptions: any = {
          color: plotLine.color,
          lineWidth: (plotLine.lineWidth || 2) as 1 | 2 | 3 | 4,
          priceLineVisible: false,
          lastValueVisible: true,
          title: plotLine.title,
        };

        // Non-overlay indicators get their own price scale on the right
        if (!isOverlay) {
          seriesOptions.priceScaleId = "pine_" + plotLine.title.replace(/\s/g, "_");
        }

        const pineSeries = chart.addSeries(LineSeries, seriesOptions);

        // Non-overlay: configure its price scale to not overlap the main chart
        if (!isOverlay) {
          pineSeries.priceScale().applyOptions({
            scaleMargins: { top: 0.7, bottom: 0.02 },
            borderVisible: true,
            borderColor: plotLine.color + "40",
          });
        }

        // Align Pine values with historical dates
        // Pine output length matches OHLCV input length — align by index
        // Handle case where Pine output is shorter (warmup period) by aligning from end
        const values = plotLine.values;
        const offset = histDates.length - values.length;
        const pineData = values
          .map((v, i) => ({
            time: histDates[i + Math.max(0, offset)] as Time,
            value: v,
          }))
      ...
      // Draw hlines (only for overlay indicators on main chart, or as price lines on pine scale)
      for (const hl of pineOverlay.hlines) {
        if (isOverlay && historicalSeries) {
